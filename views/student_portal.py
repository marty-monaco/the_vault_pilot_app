...
def resolve_video_url(raw_url: str) -> str | None:
    ...
    if m:
        return f"https://www.youtube.com/watch?v={m.group(1) or m.group(2)}"
...
def get_questions(row: pd.Series, prefix: str) -> list[dict]:
    ...
    def _opts(q_suf=""):
        return [
            str(row[f"{p}_Opt{i}{q_suf}"])
            for i in (1, 2, 3)
            if pd.notna(row.get(f"{p}_Opt{i}{q_suf}")) and str(row[f"{p}_Opt{i}{q_suf}"]).strip()
        ]
    ...
    pool = [
        {
            "id": "q1",
            "text": str(row[f"{p}_Q1"]),
            "options": random.sample(q1_opts, len(q1_opts)),
            "ans": str(row[f"{p}_A1"]).strip(),
        },
        {
            "id": "q2",
            "text": str(row[f"{p}_Q2"]),
            "options": random.sample(q2_opts, len(q2_opts)),
            "ans": str(row[f"{p}_A2"]).strip(),
        },
    ]
    random.shuffle(pool)
    return pool
...
def render_student_portal(df_cms: pd.DataFrame, client: Client | None) -> None:
    ...
    if st.session_state.portal_step == "pre_test":
        ...
        answers = {
            q["id"]: st.radio(
                f"Question {i + 1}: {q['text']}", q["options"], index=None, key=f"pre_{q['id']}"
            )
            for i, q in enumerate(st.session_state.shuffled_pre)
        }
        c1, c2 = st.columns(2)
        code = c1.text_input("Class Code", value=st.session_state.active_pilot_id)
        initials = c2.text_input("Your Initials")
        ...
        if st.button("ENTER THE VAULT ⚡", type="primary", use_container_width=True):
            if not code.strip() or not initials.strip() or any(v is None for v in answers.values()):
                ...
                st.session_state.update({
                    ...
                    "student_id": initials.strip().upper(),
                    "pre_ans": answers,
    ...
    # 3. Stage: Video & Pulse Check
        ...
        if st.session_state.completed_res:
            ...
            if res["status"] == "Completed":
                st.balloons()
                badge_html = f"""
                <div style="text-align:center; padding:2rem; border:3px solid gold;
                            border-radius:1rem; background:#1a1a2e; color:#fff;">
                    <h2 style="margin:0;">{res['student_id']}</h2>
                    <p style="letter-spacing:2px; color:gold; font-weight:bold;">CERTIFIED MASTER</p>
                    <p style="font-size:1.5rem;">LIFT: {res['lift']:+d}</p>
                </div>
                """
                st.markdown(badge_html, unsafe_allow_html=True)
            else:
                st.warning(
                    f"Mastery logged! Lift: {res['lift']:+d}. "
        ...
        if v_url:
            ...
            st.warning("⚠️ Video URL missing or invalid.")
        ...
        post_ans = {
            q["id"]: st.radio(
                f"Question {i + 1}: {q['text']}", q["options"], index=None, key=f"post_{q['id']}"
            )
            for i, q in enumerate(st.session_state.shuffled_post)
        }
        ...
        if st.button("LOG MASTERY & FINISH 🚀", type="primary", use_container_width=True):
            if any(v is None for v in post_ans.values()) or st.session_state.nps_score is None:
                ...
                s_pre = sum(
                    1 for q in st.session_state.shuffled_pre
                    if st.session_state.pre_ans.get(q["id"]) == q["ans"]
                )
                s_post = sum(
                    1 for q in st.session_state.shuffled_post
                    if post_ans.get(q["id"]) == q["ans"]
                )
                lift = s_post - s_pre

                v_len = float(
                    row.get("Video_Length_Sec") or row.get("video_length_sec") or DEFAULT_VIDEO_LEN_SEC
                )
                ...
                raw_data = {
                    stage: {
                        q["id"]: {
                            "question": q["text"],
                            "selected": ans.get(q["id"]),
                            "correct_answer": q["ans"],
                            "is_correct": ans.get(q["id"]) == q["ans"],
                        }
                        for q in pool
                    }
                    for stage, ans, pool in [
                        ("pre", st.session_state.pre_ans, st.session_state.shuffled_pre),
                        ("post", post_ans, st.session_state.shuffled_post),
                    ]
                }
                ...
                if client:
                    ...
                    try:
                        client.table("pilot_mastery_logs").upsert(
                            payload, on_conflict="pilot_id,student_id,topic"
                        ).execute()
                    except Exception as e:
                        logger.error(f"Supabase logging error: {e}")

                st.session_state.completed_res = {
                    "status": status,
                    "student_id": st.session_state.student_id,
                    "lift": lift,
                }
